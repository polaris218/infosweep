...
describe('(Transaction module)', () => {
  ...
  it('should export constants', () => {
    ...
    expect(CREATE_TRANSACTION_SUCCESS).to.equal('CREATE_TRANSACTION_SUCCESS')
    expect(CREATE_TRANSACTION_FAILURE).to.equal('CREATE_TRANSACTION_FAILURE')
    expect(TRANSACTION_REQUEST).to.equal('/admin/api/transactions')
  })
  ...
  describe('Action Creator "createTransactionSuccess"', () => {
    it('should return a type with "CREATE_TRANSACTION_SUCCESS"', () => {
      expect(createTransactionSuccess(transaction)).to.have.property('type', CREATE_TRANSACTION_SUCCESS)
    })

    it('should return a type with data', () => {
      expect(createTransactionSuccess(transaction)).to.have.property('data', transaction)
    })
  })
  ...
  describe('create CREATE_TRANSACTION_SUCCESS', done => {
    const payload = { subscription_id: 1, type_of_deal: { value: 'new' }}
    let transactionApi;

    beforeEach(() => {
      transactionApi = sinon.stub(infosweepApi, 'post')
    })
    ...
    it('creates CREATE_TRANACTION_SUCCESS', done => {
      const resolved = new Promise((r) => r({data: transaction}))
      transactionApi.returns(resolved)

      const expectedAction = [
        { type: CREATE_TRANSACTION_SUCCESS, data: transaction }
      ]

      const store = mockStore({ transaction: {} })

      return store.dispatch(createTransaction(payload))
        .then(() => {
  ...
  describe('(Reducer)', () => {
    ...
    it('should handle CREATE_TRANSACTION_SUCCESS', () => {
      const transactionState = reducer([transaction], { type: CREATE_TRANSACTION_SUCCESS, data: newTransaction })
      const expectedState = [newTransaction, transaction]
      expect(transactionState).to.eql(expectedState)
    })
  })
})
